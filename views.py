...
from BookStack.books.models import Book,Tag
import re
...
def upload_result(request):
    ...
    try:
	#don't forget column user 
	b1 = Book.objects.create(title=bookdict.get('title','').encode('UTF-8'),author=bookdict.get('author',''),publisher=bookdict.get('publisher',''),publish_date=datetime.date(pubdate[0],pubdate[1],pubdate[2]),pages=int(bookdict.get('pages','0')),ISBN=bookdict.get('isbn13',''),image_src = image_src,user=request.user)
	# add tags
	for tag in tags:
	    tt = Tag(tag = tag)
	    try:
